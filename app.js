...
(async function(){
  ...
  if(document.body.classList.contains('page-main')){
    ...
    const entregaAInput = document.getElementById('entregaAInput');
    ...
    confirmVariosBtn.addEventListener('click', () => {
      const entregaA = entregaAInput.value.trim();
      if (!entregaA) { showMessage('Debe especificar a quién se entrega', 'error'); return; }
    ...
    guardarFirma.addEventListener('click', async ()=>{
      ...
      if (currentBatchToDeliver.length > 0) {
        ...
        try {
          for (const p of currentBatchToDeliver) {
            p.estado = 'entregado'; p.firma = firmaDataURL; p.idFoto = idFotoDataURL; p.entregadoPor = entregadoPor; p.entregadoEn = entregadoEn; p.fotoEntregadoPor = userFoto; p.entregaA = entregaA;
            await putItem('paquetes', p);
            await addItem('historial',{paqueteId:p.id,estado:'entregado',usuario:entregadoPor,fecha:entregadoEn,nota:`Entrega en lote - Se entregó a: ${entregaA}`});
          }
          if (dom) { const doms = await getAll('domicilios'); domInfo = doms.find(d => d.calle === dom); }
          const comentariosMsg = comentarios ? `\nComentarios: ${comentarios}` : '';
      ...
      domicilioInput.value=''; 
      comentariosPaquete.value = ''; fotoPreview.innerHTML = ''; idPreview.innerHTML = ''; idFotoInput.value = ''; entregaAInput.value = '';
